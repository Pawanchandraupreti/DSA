...
struct Mat{ ll a[2][2]; Mat(){a[0][0]=a[0][1]=a[1][0]=a[1][1]=0;} };
Mat mul(const Mat &x, const Mat &y){ Mat r; for(int i=0;i<2;++i) for(int j=0;j<2;++j) for(int k=0;k<2;++k) r.a[i][j]=(r.a[i][j]+ x.a[i][k]*y.a[k][j])%MOD; return r; }
